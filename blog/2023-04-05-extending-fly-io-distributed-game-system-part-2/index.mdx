...
The primary technical concept behind GameBox is, as lifted from the previous post, is about replacing a key GenServer implementation with an Extism plug-in. Like its inspiration, GameBox is built with the [Phoenix](https://www.phoenixframework.org/) framework and the GenServer we targeted to extend is [LiveView](https://hexdocs.pm/phoenix_live_view/Phoenix.LiveView.html#content).  The LiveView module has a pretty simple API:
