...
### Extending Fly.io's Distributed Turn-Based Game System, Part 1

Since I built [Elixir support](/docs/integrate-into-your-codebase/elixir-or-erlang-host-sdk) into Extism,
I've been thinking about how to demonstrate the power of combining Extism and the Elixir ecosystem.

I wanted to explore wrapping a Wasm module in an Erlang process and see what kind of interesting things could you create
when combining it with OTP. Along the way, I discovered that the programming model of an Extism plugin maps almost perfectly
...
extend this [turn-based game engine created by Fly.io](https://fly.io/blog/building-a-distributed-turn-based-game-system-in-elixir/).
...
Fecth the deps and compile them:
...
You can think of a Context as an arena of plugins. When a plugin is loaded, the context owns
...
With that in mind, I implemented this barebones GenServer in `lib/game_box.ex`:
...
Now we're doing the same as before, but instead of invoking a function on the plugin we're 
invoking it on a pid. What does this inversion give us exactly? Well, because it's a pid, it can live anywhere
in your cluster. And it can be [supervised](https://hexdocs.pm/elixir/Supervisor.html) or [registered](https://hexdocs.pm/elixir/Registry.html).
...
In the next post we will show how we can use this PluginServer to replace an important part of the Phoenix ecosystem, the [Phoenix.LiveView](https://hexdocs.pm/phoenix_live_view/Phoenix.LiveView.html) module.
