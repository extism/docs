...
So naturally I built [Elixir support](/docs/integrate-into-your-codebase/elixir-or-erlang-host-sdk) into Extism.
...
when you combined this with OTP.
...
Okay, how do we turn this into an actor? I think the natural fit is a [GenServer](https://hexdocs.pm/elixir/GenServer.html).
There would be two approaches:

1. Wrap the Extism Context in the Genserver
    * We could store the Context as the state
    * We can provide callbacks to load and unload plugins
    * We can provide callbacks to lookup and call those plugins
2. Wrap the Extism Plugin in the Genserver
    * We can store both the context and the plugin as state
    * We can provide callbacks to load and reload the plugin code
    * We can provide callbacks to call functions on the plugin

:::info What is a Context?

You can think of a Context right now as just an arena of plugins. When a plugin is loaded, the context owns
...
Let's prove this by creating 2 nodes:

> # TODO

You could imagine creating a cloud of serverless functions with this. From any node
in the cluster you can load up a Wasm plugin on any other node and invoke functions on it.
Something like [riak_core](https://github.com/basho/riak_core) or
 [partisan](https://github.com/lasp-lang/partisan) could be used to create a highly available system. e.g., 
you could ensure that no less than `n` plugins exist on your cluster at any given time. Or perhaps you could
build a distributed database with Wasm compute.
