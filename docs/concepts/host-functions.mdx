...
to run on the plug-in's behalf? This is where Host Functions become very powerful. 
...
Generally, you will need to understand how to think about mapping the data types in your Host Function's
signature to the ones available in WebAssembly. The following table describes these data types. 

> **NOTE:** The type information may vary slightly per SDK, so check out the SDK-specific documentation
for more precise mapping.

| Host Function Type | C | C++ | Node | Go | Python | Rust | OCaml |
| ------------------ | - | --- | ---- | -- | ------ | ---- | ----- |
| `I32` | ? | ? | ? | ? | ? | ? | ? |
| `I64` | ? | ? | ? | ? | ? | ? | ? |
| `F32` | ? | ? | ? | ? | ? | ? | ? |
