...
host's database and receive the results? This is where Host Functions become very powerful, creating 
a more "bi-directional" relationship between host programs and plug-ins. Rather than only the host 
application invoking plug-in code, Host Functions make it possible for a plug-in to invoke host 
application code.
